...
- Advisory feedback on AI Packs is not available in the maintainer-only launch mode. A future governed mode may expose the modeled uncounted feedback path.
...
- Approve seals. AI Pack review is not launched for analysts in the current maintainer-only private mode.
...
OSI's own database is treated as a cache. Solana is the source of truth. Credential enforcement is active: a review carries zero governance count and zero weight unless the exact SAS credential is verified live or represented by a fresh trusted verification record. An RPC outage produces a pending or unavailable state, never invented authority. Expired, absent, revoked, wrong-program, wrong-issuer, wrong-schema, and wrong-credential results do not count.
...
- Finalize outcomes that already earned their analyst quorum: the server-computed winning Report and the seal after a clean challenge window. AI Pack review and approval are not available in the current private launch mode.
...
An AI Pack is a structured brief generated by a language model from server-approved Case evidence. Production currently runs a deliberately narrow **maintainer-only private mode**. The configured admin wallet and authenticated Supabase maintainer identity must both pass. Public users, Case owners, probationary and verified analysts, and either half-maintainer cannot discover Pack content, generate a Pack, or spend provider quota.
...
- **Generation is controlled.** Only the full double-gated maintainer can generate or inspect private drafts, under strict rate limits, daily quotas, cooldowns, input/output caps, timeout, idempotency, and cost caps. The provider key exists only on the server.
- **No production publication path exists.** Review, owner feedback, approval, publication, download, and public discovery are disabled and absent from ordinary UI. Generated content remains a private operational draft.
- **The future governed model stays strict.** If launched separately, public release will still require at least 2 independent SAS-valid analysts with combined weight at or above 2.50, creator exclusion, and maintainer finalization. Bootstrap mode can never substitute for this quorum.
...
For every transfer, OSI first shows the exact mainnet network, purpose, recipient manifest, amounts, canonical Memo, irreversibility, and no-custody warning. Phantom remains available and is required for an atomic multi-recipient manifest. An exact single-recipient reward or support intent also offers Solana Pay:

- Desktop shows a locally rendered QR and a copyable link.
- Mobile shows an explicit “Open in a compatible wallet” action and the copy fallback.
- The wallet never opens automatically.
- Scanning or opening the link never means paid. OSI finds the transaction through the unique server-bound reference and applies the same finalized verification and atomic receipt commit used by Phantom.
- An expired, changed, reused, cross-target, wrong-payer, wrong-recipient, wrong-amount, wrong-Memo, wrong-network, failed, or non-finalized transaction is not paid.
...
| Each SOL transfer (reward, support) | Exactly 1 wallet approval, through Phantom or a compatible Solana Pay wallet for a single recipient |
