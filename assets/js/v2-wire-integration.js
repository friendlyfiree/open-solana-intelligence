...
(function(){
  ...
  var state={
    ...
    summaryManual:false,summaryDrafting:false
  };
  ...
  async function submitWireReview(versionRef){
    if(state.reviewBusy||!validVersion(versionRef))return;var root=document.querySelector('[data-wire-queue-card="'+versionRef+'"]');if(!root)return;var decision=root.querySelector('[data-wire-review-decision]').value,rationaleField=root.querySelector('[data-wire-review-rationale]'),rationale=String(rationaleField.value||'').trim(),note=String(root.querySelector('[data-wire-review-note]').value||'').trim(),requiresRationale=decision==='reject'||decision==='request_revision';rationaleField.setCustomValidity('');if(requiresRationale&&rationale.length<10){rationaleField.setCustomValidity('Add a public-safe rationale of at least 10 characters for this decision.');rationaleField.reportValidity();rationaleField.focus();return;}if(!rationale)rationale=decision==='approve'?'The exact Wire version and its evidence were reviewed.':'The analyst abstained from a weighted decision on this version.';var reasonCode=decision==='approve'?'wire_evidence_reviewed':decision==='reject'?'wire_evidence_insufficient':decision==='request_revision'?'wire_revision_requested':'wire_analyst_abstained';state.reviewBusy=true;try{var wallet=await ensureWallet(),review={version_public_ref:versionRef,decision:decision,reason_code:reasonCode,public_rationale:rationale,private_note:note||null},prepared=await api({op:'prepare_wire_review',wallet:wallet,review:review,idempotency_key:randomKey()});if(!prepared.already_committed){var signature=await signMessage(prepared.message);await api({op:'commit_wire_review',wallet:wallet,nonce:prepared.nonce,message:prepared.message,signature:signature,review:review});}if(typeof window.osiV2RemoveDraft==='function')window.osiV2RemoveDraft(queueDraftKey(wallet));showToast('Wire review recorded with wallet-signed server proof.');await openWireQueue();}catch(error){showToast(userError(error));}finally{state.reviewBusy=false;}
  }
