...
### `support_events`
Authoritative voluntary native-SOL transfer row. It preserves the typed recipient (`report_author`, eligible `analyst`, counted reviewer, or the separately typed public `maintainer`), sender, exact bigint lamports, tx signature, state, server-derived bounded `recipient_manifest`, manifest hash, optional Case/exact published Report context, finality metadata, and exact event receipt. Report, analyst, and reviewer support uses 1–4 unique non-self recipients within one target context. Maintainer support is a distinct one-recipient intent: the server derives the singleton public maintainer profile, requires its wallet to equal the current configured `admin_wallet`, and rejects self-support. It never creates analyst standing. Support is never joined into ranking, reputation, eligibility, weight, recommendation, priority, or governance calculations.
...
| Report `body_private` | author+analyst+maintainer | always restricted; never an anonymous fallback |
| Report `content_public_safe` | authorized while pending | public after exact-version publication when supplied; honest nullable state otherwise |
...
- **Report public content** = the optional `content_public_safe`, public evidence references and publication proof selected by `case_reports.current_published_version_id` (advances only via the quorum publication transition; `body_private` remains restricted and prior versions stay auditable via their own `published_at`/`superseded_at`).
