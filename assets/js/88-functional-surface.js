...
(function(){
  ...
  function opsText(value){return typeof window.osiT==='function'?window.osiT(String(value)):String(value);}
  ...
  function renderAiPackOperations(host,status){
    ...
    appendMetric(summary,opsText('Provider'),opsText(status.provider_configured===true?'configured':'blocked'));
    ...
    if(!cases.length){
      ...
      cases.forEach(function(item){
        ...
        var detail=make('div','');detail.appendChild(make('b','',String(item.public_ref||opsText('Case unavailable'))+' / '+String(item.title||opsText('Untitled Case'))));
        ...
        detail.appendChild(make('span','',String(item.stage||'unknown')+' / '+opsText(prerequisite)));
