...
(function(){
  ...
  function bootstrapReason(code){
    ...
    };return reasons[String(code||'')]||'The server-derived bootstrap prerequisite is not met.';
  }
  function resolution(item){
    ...
    var standardFinalize=selectionStandard.can_finalize===true
      ? '<button class="osi-action primary" type="button" onclick="osiV2GovernanceFinalizeResolution(\'standard\')">Finalize standard quorum leader</button>'
      : '<button class="osi-action" type="button" disabled title="'+esc(!selectionTask?reviewTaskRequiredMessage():'Requires a unique server-derived analyst quorum leader and the full maintainer double-gate')+'">Standard finalization unavailable</button>';
    var bootstrapPanel='';
    if(selectionCapability){
      bootstrapPanel='<div class="osi-governance-seal"><h4>Maintainer bootstrap (D17)</h4><p>Tier '+esc(label(selectionBootstrap.tier||'disabled'))+' &middot; '+esc(selectionBootstrap.eligible_analyst_count==null?'unavailable':selectionBootstrap.eligible_analyst_count)+' eligible analysts. This channel is not an independent analyst quorum.</p>'
        ...
        +(selectionBootstrap.can_finalize===true?'<button class="osi-action primary" type="button" onclick="osiV2GovernanceFinalizeResolution(\'bootstrap\')">Finalize exact version via D17</button>':'<button class="osi-action" type="button" disabled title="'+esc(bootstrapReason(selectionBootstrap.reason_code))+'">D17 finalization unavailable</button>')+'</div>';
    }
    ...
    if(row&&row.state==='in_challenge_window'){
      ...
      seal='<div class="osi-governance-seal"><h4>Process seal</h4>'+progress(sq.approve_count||0,sq.approve_weight||0,sq.required_count||2,sq.required_weight||2.5)
        ...
        +(sealStandard.can_finalize===true?'<button class="osi-action primary" type="button" onclick="osiV2GovernanceFinalizeSeal(\'standard\')">Memo-anchor standard process seal</button>':'<button class="osi-action" disabled title="Standard analyst seal quorum is not ready.">Standard seal unavailable</button>')
        +(sealCapability?'<div class="osi-case-note">D17 '+esc(label(sealBootstrap.tier||'disabled'))+' &middot; '+esc(sealBootstrap.actual_support_count||0)+' / '+esc(sealBootstrap.required_support_count||0)+' analysts &middot; '+esc(Number(sealBootstrap.actual_support_weight||0).toFixed(2))+' / '+esc(Number(sealBootstrap.required_support_weight||0).toFixed(2))+' weight</div>'+(sealBootstrap.can_finalize===true?'<button class="osi-action primary" type="button" onclick="osiV2GovernanceFinalizeSeal(\'bootstrap\')">Memo-anchor seal via D17</button>':'<button class="osi-action" disabled title="'+esc(bootstrapReason(sealBootstrap.reason_code))+'">D17 seal unavailable</button>'):'')+'</div>';
    }
  ...
  function challenges(item){
    ...
    var targetCopy='<div class="osi-case-note">Target: Resolution '+esc(resolution.public_ref)+' and its bound winning Report version '+esc(resolution.winning_report_version_ref||'unavailable')+'.</div>';
    var submit=active&&walletPubkey&&caps.resolution_lifecycle_writes_enabled===true&&eligibleEvidence.length
      ? '<div class="osi-governance-compose"><h4>Submit a challenge</h4>'+targetCopy+'<label>Public-safe summary<textarea id="osi-challenge-summary" minlength="20" maxlength="10000" placeholder="Describe the challenge without restricted material."></textarea></label><label>Existing public evidence<select id="osi-challenge-evidence">'+evidenceOptions+'</select></label><label>Restricted detail<textarea id="osi-challenge-detail" maxlength="10000" placeholder="Optional restricted context."></textarea></label><button class="osi-action primary" type="button" onclick="osiV2GovernanceSubmitChallenge()">Sign and submit challenge</button></div>'
      : active&&walletPubkey&&caps.resolution_lifecycle_writes_enabled===true
      ? '<div class="osi-state-message"><b>Challenge submission unavailable</b><span>No public, approved evidence is linked to this Case or its winning Report version. Challenge intake cannot accept a typed internal ID; add/evaluate evidence through the separately governed evidence workflow first.</span></div>'+targetCopy
  ...
  function governanceFinalizeResolution(channel){
    ...
    if(!allowed){showToast('The server-derived finalization prerequisite is no longer met. Refresh My Reviews.');return;}
  ...
  function governanceFinalizeSeal(channel){
    ...
    var capability=state.activeReviewTask&&state.activeReviewTask.finalizationCapability;if(!capability)return;
    var bootstrap=String(channel||'')==='bootstrap';var allowed=bootstrap?capability.bootstrap&&capability.bootstrap.can_finalize:capability.standard&&capability.standard.can_finalize;
    if(!allowed){showToast('The server-derived seal prerequisite is no longer met. Refresh My Reviews.');return;}
  ...
  function governanceSubmitChallenge(){
    ...
    if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(evidence)){showToast('Select public, approved evidence linked to this Case or its winning Report version.');return;}
