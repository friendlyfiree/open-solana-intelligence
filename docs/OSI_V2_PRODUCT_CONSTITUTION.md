...
- **P3 — No self-decisive authority.** No actor may be the sole authority for publication, rejection, AI-Pack approval, or resolution — of their own contribution or in general. Critical outcomes require **≥ 2 independent analyst wallets**. **Cold-start amendment (2026-07-25, D17):** before an independent analyst network exists this threshold cannot be met at all, so a bounded, self-decaying exception applies to exactly four outcomes — Case Report publication, Wire Report publication, resolution / winning-Report selection, and seal — under the tier ladder in D17, which restores the full ≥ 2 requirement at 50 live eligible analysts and never applies to AI-Pack approval, challenge accept/reject, or Case initial rejection. Nothing produced through it may be presented as independent analyst consensus: every such receipt carries `decision_channel='maintainer_bootstrap'` and is displayed as maintainer bootstrap. The prohibition on implying consensus that did not occur (§3) is unaffected and absolute.
...
- **P5 — Maintainer is a finalizer only where the locked thresholds require it, never the truth.** Per the locked thresholds (D5, `OSI_V2_VOTING_REPUTATION_MODEL.md §5`), a maintainer signature is an **explicit additional requirement for exactly three outcomes: resolution / winning-Report selection, AI-Pack approval, and seal.** Case initial open has one narrow alternative: either the analyst count+weight path passes, **or** a full maintainer whose configured admin wallet and Supabase maintainer identity both pass records `approve_open` and anchors `CASE_OPENED`; this is an alternative path, not a maintainer gate added to the analyst path. Case initial rejection, Case Report and Wire Report publication/rejection, and challenge accept/reject remain analyst-quorum outcomes. The maintainer never turns initial open into a truth or guilt decision, invents a winner, or overrides any other quorum result; a stricter no-maintainer fallback for the three maintainer-gated outcomes is designed but gated behind product-owner approval (D3). **Cold-start amendment (2026-07-25, D17):** while the live eligible-analyst count is below 50, the full double-gated maintainer may additionally finalize Case Report publication, Wire Report publication, resolution / winning-Report selection, and seal, on the tier ladder in D17 — alone below 20, with one independent analyst at 20–29, with two independent analysts and the reduced Σweight gate at 30–49, and not at all at 50 or more. The tier is computed by the server from live eligible analysts (`osi_private.osi_v2_bootstrap_tier()`), never by a manual flag flip, so the exception decays on its own. It is gated by `OSI_V2_BOOTSTRAP_MAINTAINER_QUORUM_ENABLED`, requires both maintainer gates exactly as every other maintainer action does, and can never be used by the maintainer for a version they authored, nor let them count as one of its analyst reviewers. Challenge accept/reject stays analyst-quorum-only in every tier, so a maintainer can never rule on a challenge to their own bootstrap decision.
...
A signed contribution of findings that **belongs to exactly one Case**. Authored by the Case owner, an ordinary wallet, a researcher, or an analyst. **Case Report privacy:** a Case Report version is **private** (author + verified analysts + maintainer only, via proof-bound access). Publication makes the exact version's process metadata, public evidence references, proof and optional `content_public_safe` summary public; `body_private` remains restricted and is never an anonymous fallback. A missing public-safe summary is an honest nullable state, not a publication blocker. Unpublished versions never leak. A Report author may **never** review their own Report, and the Case owner may never create a decisive self-review path. One Report version may be chosen as the Case's **winning / resolution Report**; it remains an immutable contribution record forever.
...
### 5.6 Wallet profile
An optional, owner-controlled public identity for any connected wallet. Publication is explicit and reversible; public-Case attribution requires a second explicit consent. Creation and edits are wallet-signed, exact-revision-bound events. The profile contains identity only and can never grant, imply, or modify analyst standing, voting weight, SAS credentials, or maintainer authority. Existing legacy or analyst identity is not automatically imported because ownership and publication consent cannot be inferred.
...
- **B — Voluntary support:** any connected wallet may support a currently published Report author, eligible verified analyst, counted eligible reviewer, or the one public maintainer profile whose wallet still exactly matches the configured `admin_wallet`. Recipients are server-derived, self-support is rejected, and an optional atomic batch is limited to four recipients for one exact Report version; maintainer-profile support is always a separate single-recipient intent and never represents analyst eligibility. It is direct, non-custodial, and **has zero influence** on review, ranking, recommendation, priority, reputation, eligibility, voting power, publication, resolution, or maintainer authority.
