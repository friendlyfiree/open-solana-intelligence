...
has("only exact safe pending sets", /"\$\{GOVERNANCE_VERSION\} \$\{REPORT_BODY_VERSION\}"\|"\$\{GOVERNANCE_VERSION\}"\|"\$\{REPORT_BODY_VERSION\}"\|""/);
has("extra remote migrations denied", /Production contains migration versions absent from current main/);
has("expired Management token has a pinned regional fallback", /Management API answered/);
has("fallback allows only the two regional pooler generations", /aws-0-ap-southeast-2\.pooler\.supabase\.com aws-1-ap-southeast-2\.pooler\.supabase\.com/);
has("fallback probe is read-only", /psql -Atqc 'select 1'/);
has("pooler tenant is exact", /PGUSER="postgres\.\$\{EXPECTED_PROJECT_REF\}"/);
has("dry-run uses direct database credentials", /supabase db push[\s\S]*--db-url[\s\S]*--password "\$SUPABASE_DB_PASSWORD" --include-all --dry-run/);
has("dry-run exact filenames", /Dry-run mismatch/);
has("additive push uses the same reviewed route", /--password "\$SUPABASE_DB_PASSWORD" --include-all --yes/);
