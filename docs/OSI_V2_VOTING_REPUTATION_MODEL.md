...
These are the locked initial defaults (`OSI_V2_OPEN_DECISIONS.md` D5); exact numbers are tunable via `osi_config` (measurement detail) without schema change.

**Live deviation (D21, 2026-08-07).** The two standard publication weight gates run at **`1.00`**, not the `2.00` in the table above: `OSI_V2_REPORT_STANDARD_MIN_WEIGHT` and `OSI_V2_WIRE_STANDARD_MIN_WEIGHT`. Every analyst on the live roster is still at the 0.50 probationary floor, so `2.00` required four unanimous approvers and the whole network in agreement could not publish. **Both count gates remain at 2** — that is the P3 floor and no quorum function accepts a configured count below it. High-risk publication, rejection, challenge, resolution, AI-Pack and seal are unchanged. See D21 for the sign-off and the restore condition. **Safety/moderation blocking is NOT on this table** — it is a maintainer/server-policy action requiring no factual analyst quorum (State Machines §1); it must never be presented as a factual review outcome.
