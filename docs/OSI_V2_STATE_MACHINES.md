...
| in_review→published | quorum (**no maintainer required**) | EF ≥N_min + weight, author excluded | ≥2 | ≥2.00 (std) | Memo `REPORT_PUBLISHED` (names exact version) | version.lifecycle_state=published, `published_at` set, `publication_receipt_id` set; header.`current_published_version_id` advances to this version | publication metadata, public evidence/proof and optional public-safe summary public; `body_private` restricted | correction via new version (below) |
...
| publish the corrected version | quorum (**no maintainer required**) | EF ≥N_min + weight, author excluded | ≥2 | ≥2.00 (std) | Memo `REPORT_PUBLISHED` (new exact version) | new version published; **prior published version keeps its `published_at`, gains `superseded_at`+`superseded_by_version_id`**; header.`current_published_version_id` advances | new public metadata/evidence/proof and optional public-safe summary visible; private bodies remain restricted; old public process history preserved | – |

**Cold-start alternative for the publish rows (D17, Constitution P3/P5 amendment 2026-07-25):** the `≥2 / ≥2.00` gate above is the standard channel and stays the norm. While the server-computed live eligible-analyst count is under 50, a full double-gated maintainer may finalize publication on the D17 tier ladder — alone below 20, with one independent analyst at 20–29, with two plus the reduced Σweight gate at 30–49 — and never for a version they authored. Such a publication is recorded `decision_channel='maintainer_bootstrap'` and must never be displayed as independent analyst quorum. The same alternative covers resolution / winning-Report selection and seal. It does **not** reach rejection, challenge accept/reject, or AI-Pack approval, which stay analyst-quorum outcomes in every tier.
