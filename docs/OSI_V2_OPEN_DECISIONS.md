...
Wire-publication amendment (2026-07-18, product-owner decision): the same server-computed bootstrap tier also applies to publication of one exact immutable Wire Report version. Below 20 live eligible analysts, a full double-gated maintainer may publish alone; at 20-29 the maintainer needs one independent analyst; at 30-49 the maintainer needs two independent analysts and the existing reduced weight gate; at 50 or more the bootstrap path is retired. `osi_private.osi_v2_bootstrap_tier()` remains the single tier authority. The maintainer cannot use this path for a Wire version they authored and cannot count as one of its analyst reviewers. Every such publication receipt is permanently marked `decision_channel='maintainer_bootstrap'` and displayed as maintainer bootstrap, never analyst quorum. Wire challenge admissibility, merit review, and accept/reject remain structurally standard-channel, independent-analyst outcomes; bootstrap is unreachable for them.
...
Amendment (2026-07-17, security review before Step 0 was executed): the key model explicitly separates the **credential authority (human wallet) vs. operational issuer (dedicated low-privilege key)**. The maintainer wallet remains the credential authority: it creates the Credential and Schema with two human Phantom signatures, stays on the Credential's authorized-signer list so the product owner can always manually issue, revoke, or rotate from their own wallet, and its private key is never requested, exported, transmitted, or stored anywhere — server-side or otherwise. Automated day-to-day issuance is instead signed by a separate dedicated issuer keypair that the Step 0 tool generates fresh, client-side, from the browser's own CSPRNG (never derived from or related to the connected wallet, and never reused across runs unless the operator explicitly pastes its *public* key back in): its public key is what `osi_config` records as `OSI_V2_SAS_ISSUER_PUBKEY` — the identity the public verifier and the enforcement gate bind to, with no assumption that issuer equals credential creator — and its secret key, displayed exactly once in the tool, lives in exactly one place, the `OSI_V2_SAS_ISSUER_SECRET` Edge Function secret. That key's only power is signing attestation create/close for this one credential plus paying its small fee balance, so a server-side compromise is contained and recoverable (rotate via `changeAuthorizedSigners` from the credential authority plus a config/secret update) rather than a loss of the project's governance double-gate identity. The Credential's authorized-signer list therefore holds both keys; placing the maintainer wallet's secret in any server secret store is prohibited, permanently.

### D20 — The Wire native activation, Phase 1 intake boundary — **RESOLVED (2026-07-18, product-owner task)**
The first independently deployable Wire slice is limited to native immutable intake and the author's private workspace. `OSI_V2_WIRE_WRITES_ENABLED` is a dedicated exact flag, defaults to `false`, and fails closed when absent, malformed, or unavailable. Every initial version and revision uses `WIRE_REPORT_VERSION_SUBMITTED`, a server-issued single-use Stage-5 nonce, a canonical class-A Solana Memo from the author wallet, an exact ordered evidence-manifest hash, server-derived header/version identity, and one atomic nonce + receipt + immutable version + evidence + current-pointer transaction. The shared read-only wallet session adds only the `wire:mine` scope. One signature starts a working session with a 30-minute rolling inactivity window and an eight-hour absolute lifetime. Silent renewal presents the still-valid HMAC token, never a new wallet signature, preserves the original wallet, origin, audience, auth-subject and scope ceiling, and may narrow stale authority but cannot add scope. Disconnect, account change, genuine lapse or the absolute deadline revokes the session.

Phase 1 stores the Wire-specific public-safe title and summary separately from detailed analysis and the optional uncertainty/limits statement. All four fields remain private until a later analyst-authorized publication phase. Anonymous and ordinary authenticated database roles retain no table or RPC access, and the gateway returns only an explicit author allowlist without UUIDs, nonces, raw wallet signatures, payload hashes, unpublished author enumeration, or another wallet's lineage. The confirmed Solana transaction signature remains visible to that author so the Memo receipt can be independently verified.

Least-privileged lineage rule: a wallet may create multiple standalone Wire headers. A revision must name an exact native `OSI-WR-*` reference already owned by that wallet. Phase 1 does not adopt a legacy header by raw UUID or inferred similarity because no accepted collision-free public lineage key exists for that operation; ambiguous legacy mapping stays read-only/manual. Phase 1 enables no analyst review, publication, public Wire projection, Public Records entry, challenge mutation, support, promotion, reward, ranking, or governance effect. The D17 Wire-bootstrap amendment is therefore not active in this phase and must be recorded only with the later publication slice that implements and tests it. *Sec high · UX medium · Impl medium · Mig additive.*

Cutover and rollback delta: deploy the one additive migration plus `osi-v2-case-read` and the dedicated `osi-v2-wire` gateway, verify the private-read and flag-off smokes, then change only `OSI_V2_WIRE_WRITES_ENABLED` from `false` to `true`. A failed rollout changes only that flag back to `false`; immutable versions and receipts are retained for a reviewed forward fix, and no table, column, or populated data is dropped or rewritten. The author wallet broadcasts each class-A Memo, while `osi-v2-wire` is the trusted server actor responsible for mainnet, signer, exact-Memo, freshness, nonce, payload, replay, and receipt verification.

### D20b - The Wire Phase 2 review, publication, and public lane - **RESOLVED (2026-07-18, product-owner task)**
Phase 2 adds the complete governed lane without changing Phase 1 lineage. Eligible analysts and full maintainers receive a signed `wire:queue` projection; reviews bind one exact immutable version, are append-only, reject author self-review at the database boundary, snapshot server-derived tier and weight, use wallet-signed/server-verified receipts, and enter the SAS shadow-validation path. Standard publication requires at least two independent approving analysts and total weight at least `2.00`, configured by `OSI_V2_WIRE_STANDARD_MIN_COUNT` and `OSI_V2_WIRE_STANDARD_MIN_WEIGHT`. It has no maintainer gate. A quorum analyst broadcasts the class-A `WIRE_REPORT_PUBLISHED` Memo; the `osi-v2-wire` server verifies it and atomically advances only `current_published_version_id`. The D17 amendment above is the only alternate publication channel.

Public access is service-projected through explicit field allowlists. It exposes only a published or superseded exact version, public attribution, public evidence references, public-safe review rationale, non-restricted challenge state, confirmed support, and classified receipt fields. The successful publication transaction promotes only that exact version's linked, non-blocked Phase 1 evidence rows from pending/private to approved/public; a safety-blocked evidence row fails publication closed, and every public projection independently filters for `is_public=true` plus `moderation_state='approved'`. It never enumerates unpublished Wire headers or versions and never returns private review notes, restricted challenge detail, raw signatures, nonces, payload hashes, UUID lineage, or a private promoted Case reference. The detailed analysis and uncertainties fields become public only for the exact successfully published version; their physical Phase 1 column names do not grant broader read access.

An accepted challenge does not rewrite or unpublish history. It writes a one-time contested marker and receipt on the exact published version, preserves the immutable version and publication pointer, and displays "challenge upheld, under re-review". Any correction requires a new immutable version and a new governed publication. Challenge finalization always uses the normal independent analyst count and weight gates; maintainer bootstrap is unreachable.

Wire support uses the existing non-custodial payment verifier and is enabled only when both `OSI_V2_PAYMENT_WRITES_ENABLED` and `OSI_V2_WIRE_WRITES_ENABLED` are exactly true. The server derives the current published version's author as recipient, denies self-support, verifies finalized mainnet payer, recipient manifest, bigint lamports, Memo, freshness, and replay, and records no ranking, reputation, review-priority, voting-weight, or governance effect.

Promotion is enabled only when both `OSI_V2_WIRE_WRITES_ENABLED` and `OSI_V2_CASE_WRITES_ENABLED` are exactly true. An eligible analyst or full double-gated maintainer broadcasts `WIRE_PROMOTED`; `osi-v2-wire` verifies the Memo and creates a new private Case at `initial_review`, with no reward, the exact Wire version in its source references, and its evidence linked without modifying the original Wire Report. The promoter becomes the Case submitter; the Wire author is not silently granted Case-owner authority. Normal analyst and maintainer initial-open paths, nullable-state checks, and all later Case lifecycle gates still apply, so promotion cannot open or resolve the Case. The original Wire record remains public and cross-references the Case only after that Case itself becomes public.
